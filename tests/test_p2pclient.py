...
async def test_client_connect_failure():
    c0 = await make_p2pclient(0)
    c1 = await make_p2pclient(1)
    _, maddrs_1 = await c1.identify()
    await c0.identify()
    # test case: `peer_id` mismatches
    peer_id_random = PeerID.from_string("QmcgpsyWgH8Y8ajJz1Cu72KnS5uo2Aa2LpzU7kinSupNK1")
    with pytest.raises(ControlFailure):
        await c0.connect(peer_id_random, maddrs_1)
    # test case: empty maddrs
    peer_id_random = PeerID.from_string("QmcgpsyWgH8Y8ajJz1Cu72KnS5uo2Aa2LpzU7kinSupNK1")
    with pytest.raises(ControlFailure):
        await c0.connect(peer_id_random, [])
    # test case: wrong maddrs
    peer_id_random = PeerID.from_string("QmcgpsyWgH8Y8ajJz1Cu72KnS5uo2Aa2LpzU7kinSupNK1")
    with pytest.raises(ControlFailure):
        await c0.connect(peer_id_random, [Multiaddr(string_addr="/ip4/127.0.0.1/udp/0")])
